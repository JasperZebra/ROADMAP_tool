...
- Settings (`AVIF_OPTS`): `cqLevel: 0` (≈**100% quality** / near-lossless = large files; cqLevel is INVERTED — 0 = best/biggest, 63 = worst/smallest; ~80%→13, ~50%→32, 10%→57), `cqAlphaLevel: 0`, `speed: 10` (**fastest** preset; 0 = slowest/smallest … 10 = fastest/biggest), `subsample: 3` (**4:4:4**, no chroma subsampling — 4:2:0 (`1`) would be smaller), no lossless, `tileColsLog2/tileRowsLog2: 0` (default tile size for every image). (Subsample enum: 0=4:0:0, 1=4:2:0, 2=4:2:2, 3=4:4:4.)
- **Encoder load is promise-memoized** (`getAvifEncoder` returns `_avifEncodePromise`). This fixed a race: when importing JSON with many images, the first image started the `await import()` and every other image hit the old `_avifTried` guard while the module was still loading → got `null` → fell back to JPEG. Symptom was "only one image converts to AVIF; re-import to convert the rest." Now all concurrent callers await the one import.
- **`compressImage(dataURL, cb, maxDim, quality, forceAvif)`** — `forceAvif=true` returns the AVIF even if it's larger than the source (used by the import re-compress so embedded images are GUARANTEED true AVIF at the current settings — important because cqLevel 0 is near-lossless and the normal "never larger than input" guard would otherwise keep small PNGs as-is). Fresh uploads use `forceAvif=false` (size guard kept).
- **`compressEmbeddedImages` runs sequentially** (one image fully finishes before the next starts) so it never fires many encodes at the single WASM instance at once, and logs `re-compressing N …` / per-image `AVIF encoded …` / `done`.
...
| Trackpad pinch / `Ctrl+wheel` | Zoom in/out |
| `Ctrl+Z` | Undo |
| `Ctrl+Shift+Z` | Redo |
