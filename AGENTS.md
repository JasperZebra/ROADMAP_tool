...
`Select` and `Link/Unlink` are top-level toolbar buttons. **`Draw` and `Erase` live inside the `🎨 Canvas ▾` dropdown** (`#dd-canvas`), each followed by its controls: `brush-ui` (color, size slider, favorites 🎨) under Draw, and `erase-ui` (size slider, 🗑️ Clear) under Erase. The 🗑️ Clear button lives **only on Erase** (it clears all strokes regardless of tool). A third row, `#canvas-bg-ui`, holds the **Background** color picker + Reset. All control rows are **always visible** inside the dropdown (CSS `.dropdown-menu .brush-ui/.erase-ui { display:flex }` overrides the default hidden state) — `setTool` still toggles `.active` on `#brush-ui`/`#erase-ui` but that no longer controls visibility here. The dropdown does **not** `closeDD()` on tool select (so you can adjust the slider); it closes when you click the canvas. `setTool` also toggles `.active` on `#canvas-dd-btn` so the Canvas button stays highlighted while a draw/erase tool is active.
